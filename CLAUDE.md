...
- `optimize`, `pathfinder`, `laplace`, `stansummary`, `csv2json`, `dsl2stan`, `alist2dsl`, `stancode`, `stan2alist` — all use `OptionsLimited` (`stan2alist` adds a `-f/--force` flag).
...
- `stan2alist` — the inverse of `stancode`: reads `Results/<name>.stan`, runs the reverse chain (`StanBlockParser → StanToUlamModel → AlistTextEmitter`), and writes a McElreath `alist()` to `Preliminaries/<name>.alist.R`. Refuses to overwrite an existing alist without `--force`. Scope is the round-trip / idiomatic-McElreath Stan subset: declaration `<lower>`/`<upper>` constraints are dropped (re-derived by `AlistClassify` on the forward pass), `<offset>`/`<multiplier>` affine non-centering is stripped to the centred form, and `generated quantities`/`transformed *` blocks are dropped with a stderr warning. `for`/`while` loops (non-vectorisable indexed RHS, e.g. chimpanzees' `vector * vector` term) and multivariate distributions are out of scope and fail loud. Round-trip oracle: `alist → stancode → stan2alist → stancode` is byte-identical for vectorising models.
...
- `Stan2Alist/` — the reverse pipeline (`stan2alist`): `StanBlockParser`/`StanProgram` (line/block parser for the emitted-Stan subset), `StanToUlamModel` (reverse of `AlistToUlamModel` + `DataInference`, reconstructing `[Statement]`), `AlistTextEmitter` (`[Statement]` → McElreath `alist()` text). The Stan→`Distribution` reverse catalog (`distribution(fromStanName:args:)`, `mcElreathName(_:)`) lives in `Generator/DistributionCatalog.swift` next to the forward `name(_:)`/`args(_:)` so the tables can't drift.
