...
public func ulamPipeline(model: String,
  ...
  if fm.fileExists(atPath: alistURL.path) {
    ...
    let drivers = ((try? fm.contentsOfDirectory(atPath: paths.preliminaries.path)) ?? [])
      .filter { $0.hasSuffix(".ulam.swift") }
    if let driver = drivers.first {
      let driverURL = paths.preliminaries.appendingPathComponent(driver)
      if isStale(input: driverURL, output: stanURL) {
        if verbose { print("ulamPipeline: dsl2stan (\(driver) → \(model).stan)") }
        do {
          _ = try dsl2stan(model: model, verbose: verbose)
        } catch {
          return ("", "ulamPipeline: dsl2stan failed: \(error)")
        }
      } else if verbose {
        print("ulamPipeline: dsl2stan skipped (\(model).stan up to date)")
      }
    } else if verbose {
      print("ulamPipeline: no Preliminaries/\(model).alist.R or *.ulam.swift; skipping .stan generation")
    }
  }
