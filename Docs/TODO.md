...
- [x] ✅ **runinfo-first chain enumeration** — `Methods/RunStanSummary.swift` now exposes `chainsFromRunInfo(dirUrl:modelName:)`, which reads `<name>_output_config.json` (via `readRunInfo`) and returns the URLs cmdstan actually produced for *this* run — honouring the `id` chain-offset (batched-run workflows) and ignoring stray chain files from prior runs. Both `stanSummary` and `getSampleResult` call it first and fall back to the `chainOutputFiles` glob when the config is missing / non-sample / yields zero present files. Partial runs (4 declared, 3 on disk) return the subset and `print` a one-line warning. Seven unit tests in `RunInfoChainsTests` cover the positive / single-chain / partial / id-offset / missing-config / non-sample / no-chains-on-disk paths.
