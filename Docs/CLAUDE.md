...
Stan is a Swift Package Manager command-line tool that wraps Stan's [cmdstan](https://mc-stan.org/docs/2_37/cmdstan-guide/) toolchain on macOS. 
...
```bash
stan compile -V -I --model bernoulli
stan sample  -V    --model bernoulli num_chains=4 num_samples=1000
stan test  # runs the full cycle on Bernoulli
```
...
`Helpers/CreateDotStanModelFile.swift` and `Helpers/CreateDotJsonDataFile.swift` write a bundled bernoulli example into the target `<model>/Results/` so a fresh checkout has something to compile and sample. `compile -I` installs `<model>.stan`; `sample -I` installs `<model>.data.json`. The `test` subcommand drives the full cycle end-to-end on `~/Documents/StanCases/bernoulli/`.
...
|---|---|---|
| sample | `<model>_output_[1..4].csv` | `<model>.samples.csv` |
| stansummary | `<model>_stansummary.csv` | `<model>.stansummary.csv` |
| optimize | `<model>_optimize.csv` | `<model>.optimize.csv` |
| pathfinder | `<model>_pathfinder.csv` | `<model>.pathfinder.csv` |
| laplace | `<model>_laplace.csv` | `<model>.laplace.csv` |

- `GetSampleResults.swift` collapses the four chain files into a single `<model>.samples.csv`.
- `ExtractStanSummary.swift` reads the raw stansummary CSV and writes a lower-cased-header version to `<model>.stansummary.csv` (also normalises cmdstan `nan` tokens via `replaceNanByNil`).
...
The split is load-bearing for `optimize` specifically — `stan laplace`'s `mode=<file>` argument needs cmdstan to recognise `<model>_optimize.csv` as Stan optimize output via its `# method = optimize` header, which the in-place overwrite used to destroy.
...
**Known issue:** `stansummary` post-processing currently assumes exactly four chain files (`num_chains=4` is hard-coded as a default in `Sample.swift`). Listed in [`Docs/README.md`](README.md)'s "To do" section.

### Subcommands

Defined in `Stan.swift`:
...
- `laplace` — runs cmdstan's Laplace approximation. cmdstan requires an explicit `mode=<file>`; the orchestrator runs `stanOptimize` when `<model>_optimize.csv` is missing or doesn't carry the cmdstan `#` header (verified via a one-byte peek in `looksLikeRawOptimizeOutput`), then feeds the raw file via `mode=`. Trailing pass-through args (`stan laplace --model bernoulli draws=2000 mode=my_mode.csv`) work as for any other cmdstan subcommand. The split filename convention — raw `<model>_optimize.csv` vs clean `<model>.optimize.csv` (see Output post-processing above) — is what keeps the raw file stable across repeated invocations.
- `alist2dsl` — reads `Preliminaries/<model>.alist.R`, runs lexer → parser → lowering → classify → emitter (see **Alist module** below), writes a runnable `@main` `<Model>.ulam.swift` to the same `Preliminaries/` directory. McElreath's "first `~` statement is the likelihood" convention drives the role assignment; `dbinom(1, p)` collapses to `.bernoulli(p:)`; `σ` parameters that appear as the scale slot of a normal/cauchy/lognormal/gamma get `truncation: Truncation(lower: 0)` automatically (half-Cauchy / half-normal).
- `stancode` — in-process fast path. Same alist parser chain as `alist2dsl`, but the classified AST flows through `AlistToUlamModel.build(_:)` to a runtime `UlamModel` value, then through the existing public `stancode(_: UlamModel) throws -> String` generator, written directly to `Results/<model>.stan`. No swiftc, no subprocess. Two stancode entry points exist on the Swift API: `stancode(_ model: UlamModel) throws -> String` (pure) and `stancode(model: String, verbose: Bool) throws -> URL` (the file-based command). The label `model:` disambiguates.
- `csv2json` — reads `Preliminaries/<model>.csv` + `Results/<model>.stan`, writes `Results/<model>.data.json`. Validates that every row-data variable declared in the `.stan` schema is present in the CSV; derives `N` and `N_<col>` cardinalities from the data; fails loudly on `NA` (`Csv2JsonError.naValue` with column + row).
- `dsl2stan` — reads `Preliminaries/*.ulam.swift`, shells to `swiftc` to compile + run it, captures stdout into `Results/<model>.stan`. Locates the project source tree via `$STAN_PROJECT_ROOT` (defaults to a developer fallback path).
- `ulam` — V2.1: file-based pipeline driven by `ulamPipeline(model:cmdstan:verbose:arguments:)`. Picks the .stan-generation path by input presence: if `Preliminaries/<model>.alist.R` exists, use `stancode` (in-process, fast); otherwise, fall back to `dsl2stan` against `Preliminaries/<Model>.ulam.swift`. Then `csv2json → compile → sample`. Each step skipped when its outputs are newer than its inputs. The CLI lowercases `--model` for case-directory lookup.
- `test` (default subcommand) — drives `compile → sample → optimize → pathfinder` on `~/Documents/StanCases/bernoulli/`.

`Stan.swift` repeats the CMDSTAN-resolution block in every `run()` rather than centralising it; if the fallback path needs to change, update all of them.

### Ulam module

A Swift port of McElreath's R `ulam()` (from the `rethinking` package). Sits **above** the existing pipeline: emits a `<model>.stan` + `<model>.data.json` from a Swift result-builder DSL and hands off to the existing `compile` + `sample` machinery. Phases 1–6 + 5.5 ✅, V2.1 ✅, alist2dsl ✅, stancode ✅.
...
- [SwiftStats](https://github.com/SwiftProjectOrganization/SwiftStats) — consumes the clean `*.samples.csv` / `*.stansummary.csv` files this CLI produces. Output filesystem layout under `~/Documents/<dir>/<model>/` is the contract between the two.
